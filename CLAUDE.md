...
프롬프트에 "깊이 생각해줘", "think harder"를 포함하면 더 심층 분석한다.

### Plan Mode 활용

복잡한 작업 전 계획을 먼저 세운다. `Shift+Tab`으로 활성화.
...
대규모 리팩토링, 시리즈 포스트 기획 등에 유용하다.

### 컨텍스트 관리

| 명령어 | 용도 |
|--------|------|
| `/compact` | 대화 압축 (같은 주제 계속 시) |
| `/clear` | 히스토리 초기화 (새 주제 시작 시) |

작업 완료 후 `/clear`로 정리하면 다음 작업이 빨라진다.
