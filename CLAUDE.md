...
프로젝트에서 의미 있는 작업을 완료한 후, 다음을 확인한다.

### 자동 확인 항목
1. **관련 포스트 존재 여부**: 작업 내용과 관련된 기존 포스트가 있는가?
2. **업데이트 필요성**: 기존 포스트에 추가하거나 수정할 내용이 있는가?
3. **새 포스팅 가치**: 독립적인 포스트로 작성할 만한 내용인가?

### 판단 기준
| 상황 | 행동 |
|------|------|
| 기존 포스트와 직접 관련 | 해당 포스트에 섹션 추가 또는 내용 업데이트 |
| 기존 주제의 확장/심화 | 시리즈로 연결하거나 별도 포스트 제안 |
| 완전히 새로운 주제 | 새 포스트 작성 제안 |
| 단순 버그 수정/리팩토링 | 포스팅 불필요 |

### 동작 방식
- 의미 있는 작업 완료 시 자동으로 관련 포스트 탐색
- 업데이트가 필요하면 사용자에게 제안
- `/sync` 명령어로 명시적 확인 가능
