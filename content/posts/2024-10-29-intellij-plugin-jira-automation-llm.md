...
금요일 오후 6시. 퇴근 준비를 하던 중 슬랙 알림이 울렸다.

> "프로덕션에서 특정 차량 조회가 안 됩니다. 긴급 확인 부탁드려요."

로그를 확인하니 NPE였다. nullable 필드를 non-null로 가정한 코드가 문제였다. 수정은 5분이면 됐다. 하지만 그 다음이 문제였다.

```
1. Jira 웹 접속
2. 프로젝트 선택, 이슈 타입 선택
3. 제목 작성: "차량 조회 시 NPE 발생 수정"
4. 설명 작성: 원인, 수정 내용, 영향 범위...
5. 담당자, 우선순위 설정
6. 생성 버튼 클릭
7. 티켓 번호 복사해서 커밋 메시지에 붙여넣기
```

핫픽스는 5분, Jira 티켓 작성은 10분. 긴급한 상황에서 이 과정이 너무 번거로웠다.

"코드 변경 내용을 보고 있는데, 왜 이걸 또 타이핑해야 하지?"

Git diff에 모든 정보가 있다. 어떤 파일이 바뀌었는지, 뭘 추가했는지, 뭘 삭제했는지. 이걸 AI가 읽고 티켓을 만들어주면 되지 않을까?
