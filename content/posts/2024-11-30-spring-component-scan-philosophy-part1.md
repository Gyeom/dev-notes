...
Spring Boot 프로젝트에서 자주 듣는 질문이다. `@SpringBootApplication`은 편리하지만, 프로젝트가 커지면 어떤 컴포넌트가 어디서 등록되는지 파악하기 어려워진다. 생성자에 `@Autowired`가 붙어 있으면 IDE가 찾아주지만, 그 빈이 어떤 Config에서 왔는지는 알 수 없다.

이 글에서는 `@ComponentScan`의 암묵적 동작을 걷어내고, 의존성을 명시적으로 드러내는 방법을 다룬다. Application 클래스 하나만 보면 전체 빈 구성이 파악되는 구조다.
