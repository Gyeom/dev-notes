...
코드를 복사하면 중복이 생기고, 모놀리식으로 합치면 불필요한 의존성까지 끌고 다닌다. API 서버에 Kafka 리스너가 붙어 있을 이유가 없고, 배치 앱에 HTTP 컨트롤러가 있을 이유도 없다.

이 글에서는 `@Import`와 `profiles.include`를 활용해 하나의 코드베이스에서 여러 앱을 깔끔하게 구성하는 방법을 다룬다. 핵심은 앱별로 필요한 어댑터만 조립하는 것이다. Application 클래스만 보면 각 앱이 무엇을 사용하는지 한눈에 파악된다.
