...
복잡한 인프라도 `/start` 하나로 시작할 수 있어야 한다. 이 원칙이 전체 설계를 관통한다.

```
/start → 모든 서비스 구동 → 설정 자동 로드 → 즉시 사용 가능
```
...
Sentry, DataDog 등의 알람이 Slack에 오면, 분석부터 MR 생성까지 자동으로 처리한다.

```
장애 알람 → Claude 분석 → Jira 이슈 생성 → 브랜치 생성 → 코드 수정 → MR 생성
```
...
5분마다 GitLab MR을 자동으로 리뷰한다.

```
5분마다 실행 → MR 목록 조회 → ai-review 라벨 없는 MR 필터링 → Claude Opus 리뷰 → 코멘트 게시
```

`develop` 브랜치로 향하는 MR 중 `ai-review::done`, `ai-review::skip` 라벨이 없는 것만 대상이다. 리뷰가 끝나면 `ai-review::done` 라벨이 자동으로 붙는다.
...
| 기술 | 선택 이유 |
|------|----------|
| **Kotlin 2.1 + Spring Boot 3.4** | 코루틴으로 비동기 처리가 깔끔하다 |
